...
module InstructionSetValidation
  ...
  def valid_group?(name, group)
    pass = true
    id = 'Instructions:'
    pass = warn "#{id} Group name must be a single repeated character: #{name}" unless name.squeeze.length == 1
    group.each_key do |key|
      pass = warn "#{id} Group name #{name} does not match the length of instance #{key}" unless name.size == key.size
      pass = warn "#{id} Group instance #{key} of group #{name} must contain only 0s and 1s" if key.match?(/[^01]+/)
    end
    abort "#{id} Error Processing group #{name}" unless pass
  ...
  def validate_expansion_opcodes
    ...
    @expansion_opcodes.each do |code, num_args|
      unless opcode_in_bounds? code
        pass = warn "Instructions: Expansion opcode #{code.to_s(2).rjust(@word_size, '0')} out of bounds"
      end
      unless num_args.is_a?(Integer) && !num_args.negative? && num_args <= 2
        pass = warn "Instructions: Invalid number of args (#{num_args}) for expansion opcode #{code}"
      end
    end
    pass
  end
  ...
  def validate_instructions
    ...
    @instruction_conflicts.each_value do |instructions|
      instructions.each do |instruction|
        pass = warn "Instructions: Instruction #{instruction} conflicts with #{@instructions[instruction.binary_opcode]}"
      end
    end
    pass
  end
  ...
  def instruction_in_bounds(instruction)
    ...
    unless opcode_in_bounds? instruction.opcode
      pass = warn "Instructions: Opcode '#{instruction.opcode}' out of bounds for instruction #{instruction}"
    end
    e_opcode = instruction.expanded_opcode
    unless e_opcode.nil? || opcode_in_bounds?(e_opcode)
      pass = warn "Instructions: Expansion opcode '#{e_opcode}' out of bounds for instruction #{instruction}"
    end
    pass
  end
  ...
  def num_args_in_bounds(instruction)
    ...
    unless num_args.is_a?(Integer) && !num_args.negative? && num_args <= 2
      return warn "Instructions: Invalid number of args (#{num_args}) for instruction #{instruction}"
    end
  ...
  def proper_num_args(instruction)
    ...
    unless instruction.expanded_opcode.nil? || (instruction.args_expected == proper_arg_number)
      pass = warn "Instructions: #{instruction} has #{instruction.args_expected} " \
                  "arguments instead of #{proper_arg_number}"
    end
    pass
  end
